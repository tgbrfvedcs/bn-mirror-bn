...
const getAllWorkersPosition = async () => {
  ...
  const checkAllWorkersInTolerance = async (srcPosition, allWorkersPosition) => {
    ...
    const result = await Promise.all(list);
    console.log("checkAllWorkersInTolerance -> result", result)
    return list;
  };
  ...
  const getIncomess = async () => {
    ...
    fapiPrivate_get_allorders.filter(e=>stamp - e.time < week).forEach(element => {
      ...
      return 
        ...
        let tradeIncome = ``
        fapiPrivate_get_allorders.filter(element => element.asset === 'USDT' && element.incomeType ==='REALIZED_PNL').slice(-10).forEach(e=>
